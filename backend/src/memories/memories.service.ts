...
export class MemoriesService {
  ...
  async createMemory(
    ...
    if (createMemoryDto.categoryId) {
      const category: Category | null = await this.categoryRepository.findOne({
        where: { id: createMemoryDto.categoryId },
      });
      if (!category) {
  ...
  async updateMemory(
    ...
    const memory: Memory | null = await this.memoryRepository.findOne({
      where: { id: memoryId },
    ...
    if (updateMemoryDto.categoryId) {
      const category: Category | null = await this.categoryRepository.findOne({
        where: { id: updateMemoryDto.categoryId },
      });
      if (!category) {
        throw new NotFoundException('Category not found');
      }
  ...
  async getAllMemories(): Promise<MemoryDto[]> {
    const memories: Memory[] = await this.memoryRepository.find({
      relations: ['user', 'category', 'reactions'],
    });
  ...
  async getMemoriesByUser(userId: number): Promise<MemoryDto[]> {
    ...
    return memories.map((memory) =>
      plainToInstance(MemoryDto, memory, {
        excludeExtraneousValues: true,
      }),
    );
  }
  ...
  async unlikeMemory(memoryId: number, userId: number): Promise<void> {
    ...
    const existingReaction: Reaction | null =
      await this.reactionRepository.findOne({
        where: { memory: { id: memoryId }, user: { id: userId } },
        relations: ['memory', 'user'],
      });
  ...
  async getFilteredMemories(
    filterDto: GetMemoriesFilterDto,
  ): Promise<MemoryDto[]> {
  ...
  async getLikedMemoryIds(user: User): Promise<number[]> {
    const reactions = await this.reactionRepository.find({
      where: { user: { id: user.id } },
      relations: ['memory'],
    });

    return reactions.map((reaction) => reaction.memory.id);
  }
}
